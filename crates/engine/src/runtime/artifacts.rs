...
mod windows {
    ...
    impl ArtifactStore {
        pub(crate) fn open(_directory: PathBuf) -> std::io::Result<Arc<Self>> {
    ...
    impl OutputCapture {
        pub(crate) fn new(_store: Arc<ArtifactStore>) -> std::io::Result<Self> {
            // TODO(M2): real Windows backend
            Err(unsupported())
        }

        pub(crate) fn write(&self, _stream: OutputStream, _data: &[u8]) {
            // TODO(M2): real Windows backend
        }
    ...
    fn unsupported() -> std::io::Error {
        std::io::Error::new(
            ...
            "artifact storage is not yet supported on this platform",
        )
    }
}
