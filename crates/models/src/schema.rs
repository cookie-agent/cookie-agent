...
use oven_sdk::{
    AdapterId, ApiEndpoint, HeaderConfig, HeaderOverrides, LanguageModel, ModelCapabilities,
    ModelConfig, ModelDeclaration, ModelError, ModelId, ProviderConfig, ProviderId, ResourceId,
    SecretString,
};
...
use oven_sdk_open_responses::{
    HuggingFaceTransport, OpenResponsesAuth, OpenResponsesModel, OpenResponsesRequestOptions,
...
impl ConfiguredModel {
    ...
    pub fn build(&self, alias: &str) -> Result<ModelEntry, ModelBuildError> {
        ...
        let (model, provider_options): (Arc<dyn LanguageModel>, BTreeMap<String, Value>) =
            match &self.adapter {
                ...
                AdapterConfig::OpenResponses { settings, options } => (
                    Arc::new(OpenResponsesModel::new(ModelConfig::new(
                        provider.with_auth(self.open_responses_auth()?),
                        declaration,
                        settings.to_oven(),
                    ))?),
    ...
    fn compatible_auth(&self) -> Result<OpenAiCompatibleAuth, ModelBuildError> {
        match &self.auth {
            ...
            _ => Err(wrong_auth("OpenAI-compatible", "none or bearer")),
        }
    }
    ...
    fn open_responses_auth(&self) -> Result<OpenResponsesAuth, ModelBuildError> {
        match &self.auth {
            AuthConfig::Bearer { token } => Ok(OpenResponsesAuth::bearer(secret(token))),
...
pub enum AdapterConfig {
    ...
    OpenResponses {
        ...
        #[serde(default)]
        options: OpenResponsesOptionsConfig,
    },
}
...
impl TimeoutsConfig {
    ...
    fn open_responses(self) -> OpenResponsesTimeouts {
        OpenResponsesTimeouts {
            connect: Duration::from_secs(self.connect_seconds),
            headers: Duration::from_secs(self.headers_seconds),
            stream_idle: Duration::from_secs(self.stream_idle_seconds),
        }
...
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(tag = "type", rename_all = "snake_case", deny_unknown_fields)]
pub enum OpenResponsesTransportConfig {
    Generic { profile: String },
    HuggingFace { routing: String },
}

impl OpenResponsesTransportConfig {
    fn to_oven(&self) -> OpenResponsesTransport {
        match self {
            Self::Generic { profile } => OpenResponsesTransport::Generic {
                profile: profile.clone(),
            },
            Self::HuggingFace { routing } => {
...
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct OpenResponsesSettingsConfig {
    pub transport: OpenResponsesTransportConfig,
    #[serde(default)]
    pub timeouts: TimeoutsConfig,
...
impl OpenResponsesSettingsConfig {
    fn to_oven(&self) -> OpenResponsesSettings {
        OpenResponsesSettings {
            transport: self.transport.to_oven(),
            timeouts: self.timeouts.open_responses(),
...
#[derive(Clone, Debug, Default, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct OpenResponsesOptionsConfig {
...
impl OpenResponsesOptionsConfig {
    fn to_oven(&self) -> OpenResponsesRequestOptions {
        OpenResponsesRequestOptions {
