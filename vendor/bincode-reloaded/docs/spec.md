...
- [`with_big_endian`](https://docs.rs/bincode_reloaded/2/bincode_reloaded/config/struct.Configuration.html#method.with_big_endian)
- [`with_little_endian`](https://docs.rs/bincode_reloaded/2/bincode_reloaded/config/struct.Configuration.html#method.with_little_endian)
...
- During deserialization, values other than 0 and 1 will result in an error [`DecodeError::InvalidBooleanValue`](https://docs.rs/bincode_reloaded/2/bincode_reloaded/error/enum.DecodeError.html#variant.InvalidBooleanValue)
...
- Invalid Unicode characters can be acquired via unsafe code, this is handled as:
  ...
  - during deserialization: an error is raised [`DecodeError::InvalidCharEncoding`](https://docs.rs/bincode_reloaded/2/bincode_reloaded/error/enum.DecodeError.html#variant.InvalidCharEncoding)
...
let encoded = bincode_reloaded::encode_to_vec(tuple, bincode_reloaded::config::legacy()).unwrap();
...
bincode_reloaded currently supports 2 different types of `IntEncoding`. With the default config, `VarintEncoding` is selected.
...
See the documentation of [VarintEncoding](https://docs.rs/bincode_reloaded/2/bincode_reloaded/config/struct.Configuration.html#method.with_variable_int_encoding) for more information.
...
See the documentation of [FixintEncoding](https://docs.rs/bincode_reloaded/2/bincode_reloaded/config/struct.Configuration.html#method.with_fixed_int_encoding) for more information.
...
// SomeEnum::A
let encoded = bincode_reloaded::encode_to_vec(SomeEnum::A, bincode_reloaded::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
...
// SomeEnum::B(0)
let encoded = bincode_reloaded::encode_to_vec(SomeEnum::B(0), bincode_reloaded::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
...
let encoded = bincode_reloaded::encode_to_vec(SomeEnum::C { value: 0u32 }, bincode_reloaded::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
...
let encoded = bincode_reloaded::encode_to_vec(data, bincode_reloaded::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
...
- Duplicate keys are not checked in bincode_reloaded, but may be resulting in an error when decoding a container from a list of pairs.
...
let encoded = bincode_reloaded::encode_to_vec(list, bincode_reloaded::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
...
- bincode_reloaded will serialize the entries based on the iterator order.
...
- During serialization, the string is encoded as a sequence of the given bytes.
  - Rust strings are UTF-8 encoded by default, but this is not enforced by bincode_reloaded
- No normalization or transformation of text
- If an invalid UTF-8 sequence is encountered during decoding, an [`DecodeError::Utf8`](https://docs.rs/bincode_reloaded/2/bincode_reloaded/error/enum.DecodeError.html#variant.Utf8) error is raised

```rust
let str = "Hello 🌍"; // Mixed ASCII and Unicode

let encoded = bincode_reloaded::encode_to_vec(str, bincode_reloaded::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
...
let encoded = bincode_reloaded::encode_to_vec(arr, bincode_reloaded::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
